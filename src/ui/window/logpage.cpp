...
class LogTableModel : public QAbstractTableModel{
    class Log {
    public:
        Log(QString &type, QString &payload):type(type),payload(payload){
...
public:
    ...
    QVariant data(const QModelIndex &index, int role) const override {
        ...
        if(role == Qt::DisplayRole){
            switch (index.column()) {
                case 0: return logs.at(index.row()).getTime();
                case 1: return logs.at(index.row()).getType();
                case 2: return logs.at(index.row()).getPayload();
            }
        }
...
QWidget(parent)
{
    ...
    connect(&getInstance<Clash::RestfulApi>(), &Clash::RestfulApi::logReceived, this, [this](QString type, QString payload){
